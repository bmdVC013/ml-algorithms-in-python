...
def propagate(w, b, X, Y):
	...
	# FORWARD PROPAGATION
	A = sigmoid(np.dot(w.T, X) + b) # compute ativation (1,m)
	cost = float(-1/m) * np.sum(Y*np.log(A) + (1-Y)*np.log(1-A)) # compute cost (scalar)

	# BACKWARD PROPAGATION
	dw = float(1/m) * np.dot(X, (A-Y).T) # (n,1)
	db = float(1/m) * np.sum(A-Y) # Scalar
